 ...
 */
struct OpenfortSessionKeyStorageStruct {
    mapping(address => bool) revoked;
    mapping(address => uint256) sessionNonce;
}

contract OpenfortSessionKeyPlugin is OpenfortBasePlugin {
    ...
    constructor() EIP712("OpenfortSessionKeyPlugin", "1.0.0") {
    }

    function getPolicyStorage() internal pure returns (OpenfortSessionKeyStorageStruct storage s) {
        bytes32 position = bytes32(uint256(keccak256("openfort.account.eip4337.sessionkey")) - 1);
        assembly {
            s.slot := position
        }
    }
