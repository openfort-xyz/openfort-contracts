...
import {NewStaticOpenfortAccount} from "./NewStaticOpenfortAccount.sol";

/**
 * @title NewStaticOpenfortFactory (Non-upgradeable)
 ...
 */
contract NewStaticOpenfortFactory is BaseUpgradeableOpenfortFactory {
    constructor(address _entrypoint) BaseUpgradeableOpenfortFactory(address(new NewStaticOpenfortAccount(_entrypoint)), _entrypoint) {}
    ...
    function _initializeAccount(address _account, address _admin, address _entrypointContract, bytes calldata _data) internal override {
        NewStaticOpenfortAccount(payable(_account)).initialize(_admin, _entrypointContract, _data);
    }
}
