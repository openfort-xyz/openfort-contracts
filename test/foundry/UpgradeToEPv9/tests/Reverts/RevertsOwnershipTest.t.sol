...
contract RevertsOwnershipTest is SocialRecoveryHelper {
    ...
    function test_revert_transferOwnership_notOwner() external {
        vm.prank(_Attacker);
        vm.expectRevert("Ownable: caller is not the owner");
        _RandomOwnerSC.transferOwnership(_NewOwner);
    }

    function test_revert_transferOwnership_accountLocked() external createGuardians(1) {
    ...
    function test_revert_transferOwnership_toActiveGuardian() external createGuardians(1) {
        _executeGuardianAction(randomOwner, GuardianAction.PROPOSE, 1);
        _executeGuardianAction(randomOwner, GuardianAction.CONFIRM_PROPOSAL, 1);
    ...
    function test_revert_transferOwnership_toPendingGuardian() external createGuardians(1) {
        _executeGuardianAction(randomOwner, GuardianAction.PROPOSE, 1);
    ...
    function test_revert_acceptOwnership_noPendingTransfer() external {
        vm.prank(_NewOwner);
        vm.expectRevert("Ownable2Step: caller is not the new owner");
        _RandomOwnerSC.acceptOwnership();
    }

    function test_acceptOwnership_whileLocked() external createGuardians(1) {
        _executeGuardianAction(randomOwner, GuardianAction.PROPOSE, 1);
        _executeGuardianAction(randomOwner, GuardianAction.CONFIRM_PROPOSAL, 1);
